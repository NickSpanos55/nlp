...
def train_dataset(_epoch, dataloader, model, loss_function, optimizer,attention):
    ...
    for index, batch in enumerate(dataloader, 1):
        ...
        labels = torch.FloatTensor([[1,0] if i == 0 else [0,1] for i in labels]).to('cuda')
        #lengths = lengths.to('cuda')
...
def eval_dataset(dataloader, model, loss_function,attention):
    ...
    with torch.no_grad():
        for index, batch in enumerate(dataloader, 1):
            ...
            labels = labels.cuda()
            if(DATASET=='MR'):
                temp_labels = torch.FloatTensor([[1,0] if i == 0 else [0,1] for i in labels]).to('cuda')
            else:
                temp_labels = labels
            lengths = lengths.cuda()  # EX9
          ...
          # EX9
            ...
            loss = loss_function(outputs, temp_labels)  # EX9

            # Step 4 - make predictions (class = argmax of posteriors)
            preds = torch.argmax(outputs,dim=1)  # EX9

            # Step 5 - collect the predictions, gold labels and batch loss
            y.append(labels.cpu().numpy())

            y_pred.append(preds.cpu().numpy()) # EX9
